...
// In the loadFavorites function:
async function loadFavorites() {
    if (!currentUser) return;
    
    try {
        const response = await fetch(`favorites.php?user_id=${currentUser.id}`);
        ...
        if (data.success) {
            ...
            // Store favorites data for display
            localStorage.setItem('favoritesData', JSON.stringify(data.favorites));
        }
    } catch (error) {
...
// In the displayFavorites function:
    ...
    if (favoritesData.length === 0) {
        favoritesGrid.innerHTML = `
            <div class="empty-favorites">
                ...
                <p>Your starred books will appear here.</p>
            </div>
        `;
        return;
    }

    // Display actual favorites
    favoritesGrid.innerHTML = '';
    ...
    favoritesData.forEach(book => {
        ...
        bookCard.innerHTML = `
            <div class="book-cover">
                ${coverContent}
                <span style="font-size: 4rem; ${book.cover_image ? 'display: none;' : ''}">📚</span>
            </div>
            <button class="favorite-btn active" data-book-id="${book.book_id}" onclick="toggleFavorite(${JSON.stringify(book).replace(/"/g, '&quot;')}">
                ⭐
            </button>
            <div class="book-info">
                ...
                <div class="book-buttons">
                    <button class="btn btn-outline info-btn" onclick="showBookDetail(${JSON.stringify(book).replace(/"/g, '&quot;')})">ℹ️ Info</button>
                </div>
            </div>
        `;
